...
void Viewport::draw() const {
	//realisticaly, the HUD should be drawn via the viewport...
 // 	IoMod::getInstance().
   // writeText("Tracking: "+objectToTrack->getName(), msgPos[0], msgPos[1]);

	//write my name at the bottom
	int xpos	= Gamedata::getInstance().getXmlInt("username_loc/x");
	int ypos	= Gamedata::getInstance().getXmlInt("username_loc/y"); 
  	IoMod::getInstance().writeText("Larry Bratton ", xpos, ypos);
}
